...
import { Bell, X, CheckCircle, MessageCircle, User, Clock } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { useData } from '../contexts/DataContext';

interface Notification {
  id: string;
  type: 'swap_accepted' | 'swap_request' | 'swap_completed' | 'message';
  title: string;
  message: string;
  timestamp: string;
  read: boolean;
...
const NotificationSystem: React.FC<NotificationSystemProps> = ({ onSwapAccepted }) => {
  const { user } = useAuth();
  const { swapRequests, users } = useData();
  ...
  useEffect(() => {
    ...
    const interval = setInterval(checkForNewAcceptances, 2000);
    checkForNewAcceptances(); // Check immediately

    return () => clearInterval(interval);
  }, [swapRequests, users, user?.id, lastChecked, onSwapAccepted]);
